import {generatePasswordResetToken, hashPasswordResetToken} from "../helpers/password-reset-token.helpers.js";
...
import {ActiveUser, MessageResponse} from "../../../common/index.js";
import {
    buildPasswordResetUrl,
    buildPasswordResetEmailHtml,
    buildPasswordResetEmailText
} from "../helpers/password-reset-email.helpers.js";
import {ConfirmPasswordResetDto} from "../auth.types.js";
...
export class PasswordResetService {
    ...
    async requestAuthenticatedPasswordReset(
        ...
        const resetToken = generatePasswordResetToken(
            this.config.resetPasswordTokenBytes,
        );

        const tokenHash = hashPasswordResetToken(resetToken);

        const expiresAt = new Date(
            Date.now() + this.config.resetPasswordTokenTtlMs,
        );
        ...
        const emailOutbox = await this.database.$transaction(async (tx) => {
            await this.passwordResetTokenRepository.revokeActiveByUserId(
                activeUser.id,
                tx,
            );
