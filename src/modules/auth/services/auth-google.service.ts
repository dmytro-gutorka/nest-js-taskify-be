...
export class AuthGoogleService {
    ...
    async signIn(signInGoogleDto: SignInGoogleDto): Promise<TokensPair> {
        const googleUser = await this.googleAuthService.verifyCredential(signInGoogleDto.idToken);
    ...
    async link(activeUser: ActiveUser, linkGoogleDto: SignInGoogleDto): Promise<void> {
        const googleUser = await this.googleAuthService.verifyCredential(linkGoogleDto.idToken);
        ...
        const existingGoogleAuthByUser = await this.authRepository.findByUserIdAndProvider(
            activeUser.id,
            AuthProvider.GOOGLE,
        );
        ...
        if (googleUser.email !== activeUser.email) {
            throw new BadRequestException('Google account email must match current user email');
        }
